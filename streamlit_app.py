...
def parse_outline(data):
    """Read course metadata, objectives, official CLOs, PLOs and assessment strategy safely."""
    ...
    all_lines = paragraph_lines + [v for row in table_rows for v in row if v]
    text = "\n".join(all_lines)
    ...
    label_map = {
        ...
        "Course Teacher": "Instructor/Faculty",
    ...
    # ---------- Text fallback ----------
    patterns = {
        "Course Title": r"(?im)^Course(?:\s+Title)?\s*:?\s*(.+)$",
        "Course Code": r"(?im)^Course\s*Code\s*:?\s*(\S+)",
        "Semester": r"(?im)^(?:Year/Semester|Semester)\s*:?\s*(.+)$",
        "Program": r"(?im)^Program\s*:?\s*(.+)$",
        "Credit Hours": r"(?im)^(?:Units/Cr\s*Hrs\.?|Credit Hours)\s*:?\s*(.+)$",
        "Instructor/Faculty": r"(?im)^(?:Instructor|Teacher|Faculty)\s*:?\s*(.+)$",
        "Department": r"(?im)^Department\s*:?\s*(.+)$",
        "Academic Year": r"(?im)^Academic Year\s*:?\s*(.+)$",
        "Section": r"(?im)^Section\s*:?\s*(.+)$"
    }
    ...
    # Recognize FAST metadata when it is present.
    if any("FAST School of Management" in x for x in all_lines):
        info["Department"] = info["Department"] or "FAST School of Management"
    if any("National University of Computer" in x for x in all_lines):
        info["Institution"] = info["Institution"] or "National University of Computer & Emerging Sciences"
    for x in all_lines:
        m = re.search(r"\b(Lahore Campus|Islamabad Campus|Karachi Campus|Peshawar Campus)\b", x, re.I)
        if m:
            info["Campus"] = info["Campus"] or m.group(1)
            break

    # ---------- Course description ----------
    m = re.search(
        r"(?is)COURSE DESCRIPTION\s*(.*?)(?="
        r"Program Educational Objectives|Course Objectives|Program Learning Outcome|"
        r"Course Learning Outcomes|LEARNING OUTCOME ASSESSMENT STRATEGY|TEXTBOOK)",
        text
    )
    if m:
    ...
    # ---------- Course objectives ----------
    # First try tables: a numbered objective list following a row containing
    ...
    for table in doc.tables:
        ...
        for row in table.rows:
            vals = [clean(c.text) for c in row.cells]
            joined = " ".join(vals)
            if "Upon successful completion of the course" in joined:
                marker = True
                continue
            if marker:
                if vals and re.fullmatch(r"\d+", vals[0]):
                    ...
                    if any(k in joined.lower() for k in [
    ...
    if not objectives:
        m = re.search(
            r"(?is)Course Objectives\s*(.*?)(?="
            r"Program Learning Outcome|Course Learning Outcomes|"
            r"LEARNING OUTCOME ASSESSMENT STRATEGY|TEXTBOOK)",
            text
        )
        if m:
    ...
    # ---------- Official CLOs ----------
    # Primary: rows whose first cell is CLO# and second cell is the official statement.
    for row in table_rows:
        if not row:
            continue
        cid = normalize_clo(row[0])
        if cid:
            candidate = clean(row[1]) if len(row) >= 2 else ""
    ...
    # More robust table scan: if CLO and wording are split across columns,
    # use the first non-empty cell after the CLO label.
    for row in table_rows:
        for i, cell in enumerate(row):
            cid = normalize_clo(cell)
            if cid and cid not in clos:
    ...
    for i, line in enumerate(paragraph_lines):
        m = re.match(r"^\s*(CLO\s*[-_]?\s*\d+)\s*(.*)$", line, re.I)
        if m:
            cid = normalize_clo(m.group(1))
            desc = clean(m.group(2))
            if not desc and i + 1 < len(paragraph_lines):
                desc = clean(paragraph_lines[i + 1])
            if desc:
                clos[cid] = desc
    ...
    # ---------- PLOs ----------
    for row in table_rows:
        if len(row) >= 2 and clean(row[0]).isdigit():
            n = clean(row[0])
            desc = clean(row[1])
            if len(desc.split()) >= 6 and (
                "program learning outcome" in text.lower()
                or "PLO" in " ".join(row)
            ):
                plos[n] = desc
    ...
    for row in table_rows:
        if len(row) >= 2 and (
            "assessment tool" in clean(row[0]).lower()
            or "assessment tool" in " ".join(row).lower()
        ):
            assessment_strategy.append(row)

    return info, objectives, clos, plos, assessment_strategy

def read_excel(data, filename=""):
    if str(filename).lower().endswith(".csv"):
        return {"CSV": pd.read_csv(io.BytesIO(data), header=None)}
...
def parse_template_obe_sheet(raw, sheet_name):
    """
    Fallback for the standard grouped OBE workbook structure used by many
    university OBE exports. It detects the template structurally and does not
    depend on a particular course, teacher, section or student.
    """
    df = raw.copy().dropna(how="all")
    if df.empty or df.shape[1] < 20:
        return None

    # Detect the student header row.
    header_idx = None
    for r in range(min(20, len(df))):
        vals = [clean(x).lower() for x in df.iloc[r].tolist()]
        if any(v in {"rollno", "roll no", "roll no.", "roll number"} for v in vals) and any(
            v == "name" or "student name" in v for v in vals
        ):
            header_idx = r
            break

    if header_idx is None:
        # The supplied OBE template normally has the student header at row 10
        # (zero-index 9). Only use it when the row has clear student labels.
        candidate = min(9, len(df) - 1)
        vals = [clean(x).lower() for x in df.iloc[candidate].tolist()]
        if any("roll" in v for v in vals) and any("name" in v for v in vals):
            header_idx = candidate

    if header_idx is None:
        return None

    # In this template, student records start immediately after the header.
    student_rows = []
    for r in range(header_idx + 1, len(df)):
        first = clean(df.iloc[r, 0]) if df.shape[1] else ""
        if first and pd.notna(pd.to_numeric(df.iloc[r, 0], errors="coerce")):
            student_rows.append(r)

    if not student_rows:
        return None

    # Detect CLO attainment percentage columns.
    # Prefer explicit "CLO Attainment" labels in the grouped header rows.
    pct_cols = {}
    for r in range(max(0, header_idx - 8), header_idx):
        vals = [clean(x) for x in df.iloc[r].tolist()]
        for c, v in enumerate(vals):
            m = re.search(r"CLO\s*[-_]?\s*(\d+)", v, re.I)
            if m and ("attainment" in v.lower() or "percentage" in v.lower() or "%" in v):
                pct_cols[f"CLO{int(m.group(1))}"] = c

    # Standard grouped export fallback: infer CLO percentage columns from
    # the repeated CLO numbers in the CLO.No row immediately above weight/mean rows.
    if not pct_cols:
        # Look for a row containing many CLO-like labels/numbers.
        for r in range(max(0, header_idx - 8), header_idx):
            vals = [clean(x) for x in df.iloc[r].tolist()]
            clo_positions = []
            for c, v in enumerate(vals):
                m = re.search(r"CLO\s*[-_]?\s*(\d+)", v, re.I)
                if m:
                    clo_positions.append((c, f"CLO{int(m.group(1))}"))
            if len(clo_positions) >= 2:
                # For each CLO, the percentage column is commonly the blank/
                # attainment column immediately following its assessment group.
                for idx, (c, cid) in enumerate(clo_positions):
                    next_c = clo_positions[idx + 1][0] if idx + 1 < len(clo_positions) else df.shape[1]
                    candidates = list(range(c, next_c))
                    # Prefer a column whose values for students lie mostly 0..100.
                    scored = []
                    for cc in candidates:
                        s = pd.to_numeric(df.iloc[header_idx + 1:, cc], errors="coerce").dropna()
                        if len(s):
                            score = int(((s >= 0) & (s <= 100)).mean() * 100)
                            scored.append((score, cc))
                    if scored:
                        pct_cols[cid] = max(scored)[1]
                if pct_cols:
                    break

    # Exact structural fallback for the common 36-column grouped OBE export.
    # This is a template-layout fallback, not course-specific data.
    if not pct_cols and df.shape[1] >= 36:
        pct_cols = {
            "CLO1": 9, "CLO2": 14, "CLO3": 22,
            "CLO4": 29, "CLO5": 34
        }
    ...
    pct_cols = {c: col for c, col in pct_cols.items() if col < df.shape[1]}
    if not pct_cols:
        return None

    # Detect overall score column: prefer G.Tot/total labels, otherwise use the
    # final numeric column in the common template.
    total_col = None
    for r in range(max(0, header_idx - 8), min(header_idx + 1, len(df))):
        for c, v in enumerate([clean(x) for x in df.iloc[r].tolist()]):
            if re.search(r"\bG\.?\s*Tot\b|\bGrand\s*Total\b|\bOverall\b|\bTotal\b", v, re.I):
                total_col = c
    if total_col is None and df.shape[1] >= 36:
        total_col = 35

    # Assessment labels/columns. First try to use labels from the grouped
    # header row; if unavailable, use a template fallback for the assessment
    # columns between CLO percentage columns.
    assessments = []

    # Search all pre-student rows for assessment labels associated with CLOs.
    for cid, pct_col in sorted(pct_cols.items(), key=lambda x: int(re.search(r"\d+", x[0]).group())):
        # Assessment columns are numeric columns before the CLO percentage
        # column since the previous CLO percentage.
        prev = 4
        previous_clos = [
            col for c, col in pct_cols.items()
            if int(re.search(r"\d+", c).group()) < int(re.search(r"\d+", cid).group())
        ]
        if previous_clos:
            prev = max(previous_clos) + 1

        candidate_cols = list(range(prev, pct_col))
        # Exclude obvious student-information columns and blank columns.
        candidate_cols = [c for c in candidate_cols if c >= 5]

        for c in candidate_cols:
            # Find the most informative label from the pre-student rows.
            label = ""
            for r in range(max(0, header_idx - 9), header_idx):
                v = clean(df.iloc[r, c])
                if v and not re.fullmatch(r"(CLO\s*No\.?|Weightage|Average|Std\.?Dev|Min|Max|Date)", v, re.I):
                    label = v
                    break
            if not label:
                # A numeric cell in the student data still represents a component;
                # label it generically rather than inventing an assessment name.
                label = f"Assessment column {c + 1}"

            s = pd.to_numeric(df.iloc[header_idx + 1:, c], errors="coerce")
            if not s.notna().any():
                continue

            # Weightage and mean are commonly in the 2nd and 3rd rows above
            # the student header. Use them only if numeric.
            weightage = np.nan
            average = np.nan
            for rr in range(max(0, header_idx - 8), header_idx):
                row_label = " ".join(clean(x).lower() for x in df.iloc[rr].tolist() if clean(x))
                if "weightage" in row_label:
                    weightage = num(df.iloc[rr, c])
                if "average" in row_label or "mean" in row_label:
                    average = num(df.iloc[rr, c])
            if pd.isna(average):
    ...
    # Build student-level dataframe, retaining identifiers for the generated
    # analysis workbook. These identifiers are never stored in the repository.
    base_headers = [clean(x) for x in df.iloc[header_idx].tolist()]
    if not any(base_headers):
        base_headers = ["Column " + str(i + 1) for i in range(df.shape[1])]

    # Identify useful identifier columns by position/name.
    id_candidates = []
    for c, h in enumerate(base_headers):
        if c <= 4 or re.search(r"roll|registration|student|name|id|section", h, re.I):
            id_candidates.append(c)
    id_candidates = list(dict.fromkeys(id_candidates))[:5]

    records = []
    for r in student_rows:
        rec = {}
        for c in id_candidates:
            h = base_headers[c] or f"Column {c + 1}"
            rec[h] = df.iloc[r, c]
        for cid, c in pct_cols.items():
            rec[f"{cid} Attainment %"] = num(df.iloc[r, c])
        if total_col is not None and total_col < df.shape[1]:
            rec["Overall Score"] = num(df.iloc[r, total_col])
        records.append(rec)

    student_df = pd.DataFrame(records)

    # If the total is missing, do not invent a course score; leave it absent.
    if "Overall Score" not in student_df.columns:
        att_cols = [c for c in student_df.columns if c.endswith("Attainment %")]
        if att_cols:
            # This fallback is only used for conventional normalized tables.
            # For the grouped OBE template, retain the exact Excel G.Tot when present.
            pass

    # Course metadata from the first few rows.
    meta = {}
    first_rows_text = []
    for r in range(min(5, len(df))):
        first_rows_text.extend([clean(x) for x in df.iloc[r].tolist() if clean(x)])
    for i, val in enumerate(first_rows_text[:-1]):
        low = val.lower()
        nxt = first_rows_text[i + 1]
        if low in {"course", "course title"}:
            meta["Course Title"] = nxt
        elif low in {"teacher name", "teacher", "instructor", "faculty"}:
            meta["Instructor/Faculty"] = nxt

    return {
        "meta": meta,
        "assessments": assessments,
        "student_df": student_df,
        "student_clo_cols": {cid: f"{cid} Attainment %" for cid in pct_cols},
        "format": "grouped_obe_template"
    }


def parse_obe_workbook(data, filename=""):
    """Read all sheets and choose the most appropriate OBE parser."""
    sheets = read_excel(data, filename)

    # 1) Standard/grouped OBE template.
    for name, raw in sheets.items():
        result = parse_original_obe_sheet(raw, name)
        if result is not None:
            return result

    # 2) Broader structural template fallback.
    for name, raw in sheets.items():
        result = parse_template_obe_sheet(raw, name)
        if result is not None:
            return result

    # 3) Conventional normalized student table.
    for name, raw in sheets.items():
    ...
    raise ValueError(
        "The uploaded workbook could not be recognized. "
        "For the standard OBE workbook, keep the original grouped layout. "
        "For a normalized workbook, include student identifiers and columns labelled CLO1, CLO2, CLO3, etc."
    )
...
def make_excel(info, clos, assessments, stats, overall, student_df, output):
    ...
    for ws in wb.worksheets:
        for col_cells in ws.iter_cols():
            if not col_cells:
                continue
            first_cell = next((c for c in col_cells if hasattr(c, "column_letter")), None)
            if first_cell is None:
                continue
            values = [clean(c.value) for c in col_cells if c.value is not None]
            width = max([len(v) for v in values] + [12]) + 2
            ws.column_dimensions[first_cell.column_letter].width = min(width, 55)
...
def make_docx(info, objectives, clos, assessments, stats, overall, student_df, chart_paths, output):
    ...
    excel_clos = {a["clo"] for a in assessments}
    for col in student_df.columns:
        m = re.match(r"^(CLO\d+) Attainment %$", clean(col), re.I)
        if m:
            excel_clos.add(m.group(1).upper())
...
if outline:
    try:
        parsed_info, objectives, parsed_clos, parsed_plos, parsed_assessment_strategy = parse_outline(outline.getvalue())
        ...
        parsed_info, objectives, parsed_clos, parsed_plos, parsed_assessment_strategy = {}, [], {}, {}, []
...
if excel:
    try:
        obe = parse_obe_workbook(excel.getvalue(), excel.name)
        st.success(
